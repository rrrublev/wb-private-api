...
Маршруты изображений и видео поставляются со встроенным снимком
`src/UpstreamRoutes.js`; библиотека не делает сетевых запросов при генерации URL.
Workflow `Update upstream routes` ежедневно сверяет карты с WB и создаёт PR при
изменении. Для ручного обновления выполните:

```bash
node scripts/update-upstreams-routes.js
```
